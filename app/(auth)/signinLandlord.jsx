...
const SignInLandLord = () => {
    ...
    const apiBaseUrl = 'http://192.168.43.125:9897/api/v1/landlordLogin';
    ...
    const submit = async () => {
        try {
            ...
            if (response.data) {
                ...
                const profileResponse = await fetch(`http://192.168.43.125:9897/api/v1/landlordProfile/${response.data.id}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                    },
            ...
            <ScrollView>
                <View className="w-full justify-center min-h-[85vh] px-4 my-6">
                    ...
                    <View style={{ alignItems: "center", marginTop: 20, flexDirection: "row", justifyContent: "space-between" }}>
                        ...
                        <Link href="../(auth)/signupLandlord" style={{ color: "#006FFF" }}>Signup</Link>
                    </View>
                    <TouchableOpacity onPress={submit} className="mt-9" disabled={isSubmitting}>
                        {isSubmitting ? (
                            ...
                            <Text style={[styles.container, isFormFilled ? styles.blueButton : styles.greyButton]}>SignIn</Text>
                        )}
                    </TouchableOpacity>
                    <Link href="../(auth)/forgotPassword" style={styles.forgot}>forgot password?</Link>
                </View>
            </ScrollView>
            <Modal
                animationType="slide"
